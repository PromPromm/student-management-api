...
my_view = AdminChangePassword.as_view("my_view")
blp.add_url_rule("/admin/change_password", methods=["PUT"], view_func=my_view)
...
class AdminList(MethodView):
    ...
    @blp.doc(
        description="Retrieve all administrators. This method can be accessed by only an admin"
    )
    @jwt_required()
    @admin_required()
    def get(self):
...
class Admin(MethodView):
    ...
    @blp.doc(
        description="Delete an administrator by id. This method can be accessed by only the SUPER admin. Takes a fresh jwt access token",
        params={"admin_id": "The id of the admin to delete"},
    )
